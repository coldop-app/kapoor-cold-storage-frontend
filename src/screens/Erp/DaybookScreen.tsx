...
const DaybookScreen = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      storeAdminApi.getKapoorDaybookOrders(adminInfo?.token || "", type, {
        sortBy,
        page: currentPage,
        limit: itemsPerPage,
      }),
  ...
  const convertToOrderFormat = (kapoorOrder: KapoorDaybookOrderData): Order => {
    if (kapoorOrder.voucher.type === "RECEIPT") {
      ...
      const convertedOrder = {
        ...
        currentStockAtThatTime: kapoorOrder.currentStockAtThatTime || 0,
        orderDetails:
          kapoorOrder.orderDetails?.map((detail) => {
            return {
              ...
              incomingOrder: {
                ...
                incomingBagSizes: detail.incomingOrder.incomingBagSizes.map(
                  (bag) => {
                    // Find the corresponding bagSize to get the quantityRemoved
                    const correspondingBagSize = detail.bagSizes.find(
                      (b) => b.size === bag.size
                    );
                    const quantityRemoved =
                      correspondingBagSize?.quantityRemoved || 0;
                    ...
                    const originalCurrentQuantity =
                      bag.quantity.currentQuantity + quantityRemoved;
          ...
          {orders.length === 0 && !isLoading && !isSearchLoading ? (
              ...
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-1">
                  {searchReceiptNumber
                    ...
                    : t("daybook.noOrdersFound")}
                </h3>
                <p className="text-gray-500">
                  {searchReceiptNumber
                    ? t("daybook.tryDifferentNumber")
                    : t("daybook.createNewOrder")}
                </p>
              </div>
            </div>
          ) : (
            <div>
              {orders.map((kapoorOrder: KapoorDaybookOrderData) => {
                ...
                // Console log the order data being sent to voucher components
